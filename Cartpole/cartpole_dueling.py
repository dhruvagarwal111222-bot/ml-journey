...
print("Dueling Double DQN training started...\n")
...
for episode in range(1000):
    ...
    for _ in range(500):
        ...
        # balance + stay centered + keep pole upright
        shaped_reward = (1.0 - 0.3 * abs(cart_pos) - 0.3 * abs(pole_angle)) if not done else -10
    ...
    if avg > best_avg:
        best_avg = avg
        torch.save(agent.model.state_dict(), "Cartpole/cartpole_dueling_best.pth")

    torch.save(agent.model.state_dict(), "Cartpole/cartpole_dueling_last.pth")
...
plt.xlabel("episode")
plt.ylabel("score")
plt.title("CartPole — Dueling Double DQN")
plt.legend()
plt.tight_layout()
plt.savefig("Cartpole/cartpole_dueling_results.png")
plt.show()
print("\ndone!")